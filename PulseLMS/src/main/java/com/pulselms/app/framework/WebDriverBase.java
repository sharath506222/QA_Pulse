...
public class WebDriverBase {
	...
	/**
	 * This method is a overloaded method of wait_internal providing flexibility
	 * in the wait time by providing timeoutInSeconds
	 * 
	 * This is an internal method
	 * 
	 * @param condition
	...
	/**
	 * Method to making the driver to wait for a time interval before proceeding
	 * further
	 * 
	 * @param timeOutInSeconds
	 */
	...
	/***
	 * @performs Drag and drop function to move an element by using pixel offset
	...
	/****
	 * @performs a click option within an element (like slider)***** At a give
	 *           pixel location
	 * 
	 ****/
	...
	protected List<String> getTextOfSimilarElements(String elementLocator) {
		...
		log.debug(
				"There are " + elementList.size() + " Similar elements(Element with locator " + elementLocator + " )");
	...
	/**
	 * Use this method to check/uncheck the checkbox irrespective of its current
	 * state
	 * 
	 * @param elementLocator
	 *            the key associated to Locator(LocatorType;LocatorValue) that
	 ...
	 *            specifies the by selector
	 * @param select
	 *            Set this to true to select the check box/radio button, false
	 *            to deselect them
	 */
	...
	protected static void highlightElement(String propKey) {
		By by = ObjectLocators.getBySelector(propKey);
		for (int i = 0; i < 2; i++) {
	...
	protected Long horizontalPageScrollXPosition(String elementLocator) {
		...
		/*
		 * Long ycord = (long) point.getY(); log.info(
		 * "Element's Position from top side Is "+ycord +" pixels.");
		 */
		return xcord;

	...
	static {
		...
		try {
			if (EnvParameters.PLATFORM.equalsIgnoreCase("Android")) {
				...
				if(EnvParameters.ANDROID_MULTICONFIG.equalsIgnoreCase("true")) {
					if(EnvParameters.ENV.equalsIgnoreCase("QA"))
						mobileStream = new FileInputStream(EnvParameters.TEST_ROOT_DIR
								+ File.separator + "ObjectRepo" + File.separator
								+ "AndroidMultiConfig" + File.separator + "QA_ObjectRepository.properties");
					else if(EnvParameters.ENV.equalsIgnoreCase("STAGE"))
						mobileStream = new FileInputStream(EnvParameters.TEST_ROOT_DIR
								+ File.separator + "ObjectRepo" + File.separator
								+ "AndroidMultiConfig" + File.separator + "STAGE_ObjectRepository.properties");
					else if(EnvParameters.ENV.equalsIgnoreCase("PROD"))
						mobileStream = new FileInputStream(EnvParameters.TEST_ROOT_DIR
								+ File.separator + "ObjectRepo" + File.separator
								+ "AndroidMultiConfig" + File.separator + "PROD_ObjectRepository.properties");
					else 
						throw new BrowserBotException(
								"Not a valid environment for this application");
			...
			if (EnvParameters.WEB_TEST.equalsIgnoreCase("true")) {
				webStream = new FileInputStream(EnvParameters.TEST_ROOT_DIR
						+ File.separator + "ObjectRepo" + File.separator + "Web_ObjectRepository.properties");
			}
